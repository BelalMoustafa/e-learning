...
<div class="container">
        <div class="row">
            ...
            if (isset($_GET['edit_id'])) {
                ...
                $fetch_stmt = $conn->prepare("SELECT * FROM `online_content` WHERE id = ?");
                $fetch_stmt->execute([$edit_id]);
                $existing_data = $fetch_stmt->fetch(PDO::FETCH_ASSOC);
            } else {
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($thumb_size > 2000000) {
                    ...
                    $update_stmt = $conn->prepare("UPDATE `online_content` SET title=?, description=?, link=?, thumb=?, data=?, time=? WHERE id=?");
                    $update_stmt->execute([$title, $description, $link, $rename_thumb, $date, $time, $id]);
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="breadcrumb__content">
                            <h3 class="title">Update Online Content</h3>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input placeholder="Online Content Title" type="text" name="title" maxlength="100"
                                    class="form-control" value="<?= $existing_data['title'] ?>" required>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label class="form-label" for="videoTitle">Online Content Time</label>
                                <input placeholder="Online Content time" type="text" name="time" class="form-control"
                                    value="<?= $existing_data['time'] ?>" required>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="date" name="date" class="form-control"
                                    value="<?= $existing_data['data'] ?>" required>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label class="form-label" for="videoDescription">Online Content Description</label>
                                <textarea placeholder="Online Content Description" name="description"
                                    class="form-control" rows="5" maxlength="1000"
                                    required><?= $existing_data['description'] ?></textarea>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label class="form-label">Online Content Link</label>
                                <input placeholder="Online Content Link" type="text" name="link" class="form-control"
                                    value="<?= $existing_data['link'] ?>" required>
                            </div>
                        </div>
                        <div class="col-sm-6">
